...
var gachaWebText = map[string]map[string]string{
	"en": {
		...
		"chapterNote": "While both pools have rewards, limited rewards share 80% and unlimited rewards share 20%. When one pool is exhausted, the other receives 100%. Monthly limits reset at 00:00 UTC−8 on the first day of each month.",
	...
	"ja": {
		...
		"chapterNote": "両方の報酬が残っている間、数量限定の報酬は合計80%、無制限の報酬は合計20%です。一方がなくなると、もう一方が100%になります。月間の残数は毎月1日 00:00（UTC−8）にリセットされます。",
