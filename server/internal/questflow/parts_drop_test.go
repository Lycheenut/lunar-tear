		...
		t.Run(name, func(t *testing.T) {
			...
			if configured {
				h.DropRewardsByQuestID = map[int32][]questdrop.Reward{10: {
					...
					{BattleDropRewardID: 1005, Weight: 10000, Guaranteed: true},
				}}
			}
			...
			for seed := int64(1); seed <= 1200; seed++ {
				...
				if configured {
					guaranteed := drops[len(drops)-1]
					if guaranteed.PossessionId != 5 || guaranteed.Count != 4 {
						t.Fatalf("guaranteed drop = %+v", guaranteed)
					}
