...
The **Activity Groups** tab combines reusable activity units. Type 1 units require at least one EventQuestChapter and can include MomBanner, NaviCutIn, exchange shops, mission terms, Variation Event Gacha, currency/ticket terms, and Tips. Type 2 units require at least one configured Premium Gacha and can include MomBanner, exchange shops, and shard terms/conversion deadlines. Groups may share units; a bulk schedule preview deduplicates shared records. Chapters, Premium Gacha, banners, cut-ins, and Tips use the selected interval; missions, Event Gacha, shops, and currency/ticket/shard deadlines end 48 hours later.

On the first enabled admin startup, existing relationships are captured once in the separate `--activity-group-config` file (default `config/activity-groups.json`; compose uses `db/activity-groups.json`). It contains `version`, `units`, and `groups`. Event Gacha times are captured once in `gacha.json.eventSchedules`; they take effect independently of activity group membership. Initialization preserves every existing schedule. Each chapter/Premium Gacha initially gets one unit and one group. Ambiguous Tips are left for manual selection; Tips with an interval belonging to exactly one chapter are included. Saved configurations, including an empty group list, are never regenerated. Individual edits no longer cascade to other records. Membership saves only write the activity group file. Bulk schedules update selected master-data rows and selected Premium/Event Gacha times in `gacha.json`. Saves check activity, Gacha, and master-data hashes to reject stale updates; a failed combined publication restores previously replaced files.
