...
The **Related Tables → Activity Resonance Restoration** editor imports complete historical costume/weapon rosters and applies selected existing weapon rules while preserving each quest's medal phase. Resonance dates follow the event schedule automatically. Events without rules can explicitly map phases and medals from another event. Titled catalogs use searchable dropdowns throughout the configuration tool. See [the activity resonance restoration guide](docs/EVENT_QUEST_BONUS_EDITOR.md).
