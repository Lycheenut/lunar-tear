...
The **Activity Groups** tab combines reusable activity units of exactly three types. Type 1 (Premium Gacha) requires at least one configured Premium Gacha and can include MomBanner, exchange shops, and shard terms/conversion deadlines. Type 2 (Record) requires at least one Record EventQuestChapter and can include MomBanner, event shops, currency terms, mission terms, NaviCutIn, and Tips. Type 3 (Variation) requires at least one Variation EventQuestChapter and can include MomBanner, the selected chapters' Event Gacha, ticket terms, mission terms, NaviCutIn, and Tips. Other chapter types are excluded. Each member type has its own section for adding and removing entries. Groups may share units; a bulk schedule preview deduplicates shared records. Chapters, Premium Gacha, banners, cut-ins, and Tips use the selected interval; missions, Event Gacha, shops, and currency/ticket/shard deadlines end 48 hours later.
