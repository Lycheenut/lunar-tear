...
| `--quest-drop-config` | `config/quest_drops.json` | Plaintext per-quest weighted and guaranteed drop configuration.    |
| `--activity-group-config` | `config/activity-groups.json` | Independent activity group membership configuration. |
| `--no-register`  | `false`          | Disable new user registrations (only already registered users can connect). |
...
The **Activity Groups** tab combines reusable activity units of exactly three types. Type 1 (Premium Gacha) requires at least one configured Premium Gacha and can include MomBanner, exchange shops, and shard terms/conversion deadlines. Type 2 (Record) requires at least one Record EventQuestChapter and can include MomBanner, event shops, currency terms, mission terms, NaviCutIn, and Tips. Type 3 (Variation) requires at least one Variation EventQuestChapter and can include MomBanner, the selected chapters' Event Gacha, ticket terms, mission terms, NaviCutIn, and Tips. Other chapter types are excluded. Each member type has its own section for adding and removing entries; MomBanner entries show image cards. The sidebar filters units by type and groups by the types of their included units. Premium shard terms automatically include same-ID conversion deadlines, displayed under the term rather than selected separately. Groups may share units; a bulk schedule preview deduplicates shared records. Chapters, Premium Gacha, banners, cut-ins, and Tips use the selected interval; missions, Event Gacha, shops, and currency/ticket/shard deadlines end 48 hours later.

On the first enabled admin startup, existing relationships are captured once in the separate `--activity-group-config` file (default `config/activity-groups.json`; compose uses `db/activity-groups.json`). It contains `version`, `units`, and `groups`. Event Gacha times are captured once in `gacha.json.eventSchedules`; they take effect independently of activity group membership. Initialization preserves every existing schedule. Each supported chapter/Premium Gacha initially gets one unit and one group. Ambiguous Tips are left for manual selection; Tips with an interval belonging to exactly one chapter are included. Version 1 activity configurations are upgraded once to version 2: mixed Record/Variation units are split, unsupported units and resulting empty groups are removed, compatible explicit membership and custom names are preserved, and Premium names are resolved from localized Gacha titles. Version 2 configurations, including an empty group list, are never regenerated. Individual edits no longer cascade to other records. Membership saves only write the activity group file. Bulk schedules update selected master-data rows and selected Premium/Event Gacha times in `gacha.json`. Saves check activity, Gacha, and master-data hashes to reject stale updates; a failed combined publication restores previously replaced files.

The **Related Tables → QuestBonus** editor replaces rosters with selected historical members or keeps current members and supplements them with selected costumes/weapons. Existing members retain their rules in supplement mode; imported weapons use selected reference rules while preserving each quest's medal phase. Explicit roster restoration uses the current event schedule; editing an event date by itself leaves resonance terms unchanged. Events without rules can explicitly map phases and medals from another event. Titled catalogs use searchable dropdowns; the table picker uses a native dropdown sorted by display name. See [the activity resonance restoration guide](docs/EVENT_QUEST_BONUS_EDITOR.md).

The **Gacha Configuration** tab manages Premium weapon pools plus Chapter and Event reward boxes. `gacha.json.banners` is also the authoritative Premium banner inventory: every entry declares its `limited_*` asset name and start/end timestamps. The operational-activities page exposes the same schedule and previews `assets/ui/gacha/{lang}/limited_*/banner.png`; MamaBanner output is derived from this configured catalog rather than used as its source. Publishing a Gacha configuration changes only the Gacha configuration. Use Activity Groups to explicitly reschedule matching MomBanner rows and other configured members. Box rewards configure grant quantity, finite stock, item weight, finite/unlimited group probability, featured display, and (for Event boxes) jackpot targets; the UI previews the effective probability from the current weights. Event boxes are ordered in the JSON because this master-data snapshot has no box-count table. Drawing every jackpot unlocks the next box, while the final box can only reset after all finite stock is drawn. Publishing validates the complete configuration, writes `--gacha-config` as readable JSON, and atomically swaps the live catalog. Chapter and Event banners without a JSON box configuration are not available for drawing, but remain visible in the editor so their configuration can be created.

The **Delivery Configuration → Quest Drops** page manages non-event quest drops. Each configured reward can appear once in the random group and once in the guaranteed group; random rewards have a positive integer weight. Publishing writes `--quest-drop-config` and atomically swaps the live catalog. For each configured in-battle drop, the server first selects `BattleDropEffectId` from the total weight of each rarity/effect tier, then selects the concrete reward by weight within that tier. Quests without an override continue through the original master-data pickup-row lottery unchanged, including duplicate rows and its original random-draw sequence.
...
| `LUNAR_ACTIVITY_GROUP_CONFIG` | Independent activity group config path (compose uses `db/activity-groups.json` in the DB volume). |
